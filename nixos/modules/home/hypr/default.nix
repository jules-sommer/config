...
let
  ...
  '';
  cfg = config.jules.window-manager.hyprland;
in with lib; {
  options.jules.window-manager.hyprland = {
    enable = mkEnableOption "Enable Hyprland";
    theme = mkOpt lib.types.attrs inputs.nix-colors.colorSchemes.paraiso.palette
      "nix-colors theme to use for Hyprland";
    plugins = {
      enable = mkEnableOption "Enable Hyprland plugins";
      splitMonitorWorkspaces =
        mkBoolOpt "Enable split monitor workspaces" false;

      extraPlugins = lib.mkOption {
        type =
          lib.types.listOf (lib.types.either lib.types.package lib.types.path);
        default = [ ];
        description = "Hyprland plugins to install, official or otherwise.";
      };
    };
  };
  ...
  config = mkIf cfg.enable {
    wayland.windowManager.hyprland = {
      enable = true;
      plugins = with inputs;
        (lib.optional cfg.plugins.splitMonitorWorkspaces
          split-monitor-workspaces.packages.${pkgs.system}.split-monitor-workspaces)
        ++ (if cfg.plugins.enable then cfg.plugins.extraPlugins else [ ]);
      xwayland.enable = true;
      systemd.enable = true;
      settings = {
        "$mod" = "ALT";
        "$multiplexer" = "-e zellij";
        "$terminal" = "alacritty";
        ...
        bind = [
          ...
          builtins.concatLists (builtins.genList (x:
            ...
            in (if cfg.plugins.splitMonitorWorkspaces then [
              # use split-workspace and split-movetoworkspacesilent
              # instead of workspace and movetoworkspace for split monitor plugin
              "$mod, ${ws}, split-workspace, ${toString (x + 1)}"
              "$mod SHIFT, ${ws}, split-movetoworkspacesilent, ${
                toString (x + 1)
              }"
            ] else [
        ...
        general = {
          ...
          "col.active_border" =
            "rgba(${cfg.theme.base0C}ff) rgba(${cfg.theme.base0D}ff) rgba(${cfg.theme.base0B}ff) rgba(${cfg.theme.base0E}ff) 45deg";
          "col.inactive_border" =
            "rgba(${cfg.theme.base0C}40) rgba(${cfg.theme.base0D}40) rgba(${cfg.theme.base0B}40) rgba(${cfg.theme.base0E}40) 45deg";
          "layout" = "dwindle";
