{ pkgs, config, lib, ... }:
let
  inherit (lib.xeta) toHomeFile;
  home = config.xeta.user.home;

  lock_config_path = "${home}/_config/swaylock/config";
  notif_config_path = "${home}/_config/swaync/config.json";

  styles_path = "${home}/_config/swaync/style.css";
  styles = ''
    * {
            font-family: Ubuntu;
            font-weight: bolder;
  ...
  palette = lib.xeta.getTheme "gigavolt";
...
(toHomeFile lock_config_path ''
  ...
  image=${settings.home}/.sysart/zoebo-pfp.jpg
  ...
  font=Ubuntu
'') (toHomeFile notif_config_path (builtins.toJSON config))
(toHomeFile styles_path styles) { }
